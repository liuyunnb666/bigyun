...
function handleSuccess(response) {
  const result = normalizeResponseBody(response)
  const msg = getResponseMessage(result)
  isUploading.value = false
  selectedFile.value = null
  uploadRef.value?.clearFiles()
  if (!isSuccessResponse(result)) {
    proxy.$alert("<div style='overflow:auto;overflow-x:hidden;max-height:70vh;padding:10px 20px 0;'>" + msg + '</div>', '导入失败', { dangerouslyUseHTMLString: true })
    return
  }
  visible.value = false
